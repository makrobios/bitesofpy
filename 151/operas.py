...
def operas_both_at_premiere(guest, composer):
    ...
    try:
        ...
        raise ValueError(f'{guest} or {composer} not found in {composers.keys()}')
    pieces = [opera.play for 
              opera in operas 
              if opera.author == composer and
              _get_date(composers[composer].born) <= _get_date(opera.date) <= _get_date(composers[composer].died) and
              _get_date(composers[guest].born) <= _get_date(opera.date) <= _get_date(composers[guest].died) ]
    return pieces 

#print(list(operas_both_at_premiere('verdi', 'wagner')))
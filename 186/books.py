...
def get_number_books_read(books_per_year_goal: int,
    ...
    datum = parse(at_date)
    if books_per_year_goal <= 0 or datum < NOW:
        raise ValueError
    week_of_year = datum.isocalendar()[1]
    return int(week_of_year / 52 * books_per_year_goal)

get_number_books_read(100, '2019-04-02')
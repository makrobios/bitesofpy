...
#passed.set_index(['Date', 'ID', 'Element'], inplace = True)
#now.set_index(['Date', 'ID', 'Element'], inplace = True)


#cond = (passed.index.month == 7) & (passed.index.day == 29) &(passed['ID'] == 'USW00014853') & (passed.Element == 'TMAX')
#passed.loc[cond]

# for pas in passed.itertuples():
#     DATE = pas.Index.replace(year=2015)
#     DATE = DATE.strftime('%Y-%m-%d')
#     ID = pas.ID 
#     ELEMENT= pas.Element
#     VALUE = pas.Data_Value
#     op = operator.gt
#     if ELEMENT == 'TMIN':
#         op = operator.lt
#     try: 
#         NOW = now.sort_index().loc[DATE,ID,ELEMENT]['Data_Value']
#         if op(NOW, VALUE):
#             pass#print(f'{pas.Index} {pas.ID} {pas.Data_Value} ==> {NOW}')
#     except KeyError:
#         pass

#select_m_d = (df.index.month == 1) & (df.index.day == 1)

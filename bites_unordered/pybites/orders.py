...
def get_year_region_breakdown(df):
    ...
    df['Year'] = df['OrderDate'].apply(lambda date: date.year)
...
def get_best_sales_rep(df):
    ...
    reps = df.groupby('Rep')['Total'].sum()
    bestrep = reps.reset_index().sort_values('Total').iloc[-1]
    return (bestrep.Rep, bestrep.Total)

def get_most_sold_item(df):
    """Return a tuple of the name of the most sold item
       and the number of units sold"""
    items = df.groupby('Item')['Units'].sum()
    bestitem = items.reset_index().sort_values('Units').iloc[-1]
    return (bestitem.Item, bestitem.Units)

    ...
    * stopwords
    ...
    def cleaned(self) -> str:
        ...
        * If a list of extract objects were given, remove those too
        ...
        """
        clean = self.txt.lower()
        if self.extra:
            for ex in self.extra:
                clean = re.sub(rf"{ex}+", " ", clean) 
        pat = '|'.join( [fr"\{p}" for p in string.punctuation] )
    ...
    def metrics(self) -> List[Tuple[str, int]]:
        ...
        * Use count to return the requested amount of the top words, defaults to 5
        ...
        """
        counter = Counter( [word for word in self.cleaned.split() if word not in STOPWORDS] )

...
def add_todo(delay_time: str, task: str,
    ...
    """
    timestamp = 0
    to_seconds = {"d": 24*60*60, "h": 60*60, "m": 60}
    times = re.sub(r"(\d+)([dhm]|s?)",r"\1:\2",delay_time)
    for time in times.split():
        number, unit = time.split(':')
        if unit != "s" and unit != "":
            number, unit = time.split(':')
            timestamp += int(number) * to_seconds[unit]
        else:
            timestamp += int(number)
    tasktime = start_time + timedelta(seconds=timestamp)
    return f"{task} @ {tasktime}"
...
def player_with_max_points_per_game():
    ...
    cur.execute("SELECT MAX( CAST( avg_points AS float)), name from players;")
    result = cur.fetchone() # (30.1, 'Michael Jordan')
    return result[1]
...
def year_with_most_drafts():
    ...
    cur.execute("""SELECT year, COUNT(name) 
                 FROM players 
                 GROUP BY year
                 ORDER BY COUNT(name) DESC LIMIT 1;""")
    most_drafts = cur.fetchone()[0]
    return most_drafts

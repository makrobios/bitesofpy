...
def display_books(books, limit=10, year=None):
    ...
    """
    count = 0
    #books = _sort_helper(books, "-rating", "year", "title", "author")
    books = sorted(books, 
                   key= lambda x: (-x.rating, x.year, x.title.lower(), x.author),
                   reverse=False)
    for idx, book in enumerate(books, 1):
        book.rank = idx
        if book.author == "Mc Fedden, Antony":
           book.author = "Fedden, Antony Mc"    
        if count == limit:
            break
        elif year:
            if book.year >= year:
...
def load_data():
    ...
    books = []
    for idx, entry in enumerate(soup.select(".book-header-title"),1):
        title = entry.find("h2").text
        try:
            author = entry.select_one(".authors a").text.strip()
            fname, *lname = author.split()
            author = f"{' '.join(lname)}, {fname}"
            rating = entry.select_one(".our-rating").text
            year =  entry.select_one(".date").text.strip(" |")
            rank = idx
...
def main():
    books = load_data()
    display_books(books, limit=None, year=2017 )

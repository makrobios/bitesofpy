...
country_lookup = {
    ...
    "Europe": [
        ...
        "Portugal", "Republic of Ireland", "Ireland", "Republic of Macedonia",
...
def _reformat_html(data):
    parser = "html.parser"
    soup = Soup(data, parser)
    entries = soup.find_all("script", {"type": "application/ld+json"})
    events = []
    for entry in entries:
        events.append(json.loads(entry.text))
    return events 
...
def get_pycon_events(data=_get_pycon_data()) -> List[PyCon]:
    ...
    events = _reformat_html(data)   
    result = []
    for event in events:
        if 'PyCon' in event['name']:
            result.append( PyCon(event['name']                             # name
                      ,event['location']['address']['addressLocality'] # city
                      ,event['location']['address']['addressCountry']  # country
                      ,datetime.strptime(event['startDate'], "%Y-%m-%d")                                # start_date
                      ,datetime.strptime(event['endDate'], "%Y-%m-%d")                                 # end_date
                      ,event['url'] )     # url
                    )
    return result
def filter_pycons(pycons: List[PyCon],
    ...
    filtered_pycons = []
    for pycon in pycons:
        start = pycon.start_date.year == year 
        end = pycon.end_date.year == year
        cont = pycon.country.title() in country_lookup[continent]
        if (start or end ) and cont:
            filtered_pycons.append(pycon)

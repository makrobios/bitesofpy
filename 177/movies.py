...
def group_by_genre(data=movie_excel_file):
    ...
    """
    data = pd.read_excel(movie_excel_file, skiprows=range(7), usecols=[2,3])  
    data.genres = data.genres.str.split('|')
    movies = data.explode('genres')
    movies = movies.loc[movies.genres != "(no genres listed)"]
    return movies.groupby(by=["genres"]).count().sort_values(by='movie',ascending=False)

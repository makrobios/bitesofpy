...
def years_gold_value_decreased(gold_prices: str = gold_prices) -> (int, int):
    ...
    """
    prices = [(int(year.split('-')[0]), float(price)) 
              for line in gold_prices.splitlines() 
              for record in line.split() 
              for year, price in zip( *[ iter( record.split(',') ) ] * 2 ) ]
    
    gold_values = {actual[0]:actual[1] - last[1] for last, actual in _pairwise( prices )}
    return ( min(gold_values, key=lambda x: gold_values[x]), max(gold_values, key=lambda x: gold_values[x]) )
